...
public class TitaniumOreFeature extends OreFeature {
	...
	public static Feature<?> feature() {
		...
		CONFIGURED_FEATURE = FeatureUtils.register("mcsmarmors:titanium_ore", FEATURE,
				new OreConfiguration(TitaniumOreFeatureRuleTest.INSTANCE, McsmarmorsModBlocks.TITANIUM_ORE.get().defaultBlockState(), 4));
		PLACED_FEATURE = PlacementUtils.register("mcsmarmors:titanium_ore", CONFIGURED_FEATURE,
				List.of(CountPlacement.of(4), InSquarePlacement.spread(),
						HeightRangePlacement.uniform(VerticalAnchor.absolute(-64), VerticalAnchor.absolute(-28)), BiomeFilter.biome()));
		return FEATURE;
	}
	...
	private static class TitaniumOreFeatureRuleTest extends RuleTest {
		...
		public static void init(FMLCommonSetupEvent event) {
			Registry.register(Registry.RULE_TEST, new ResourceLocation("mcsmarmors:titanium_ore_match"), CUSTOM_MATCH);
		}
		...
		public boolean test(BlockState blockAt, Random random) {
			if (base_blocks == null) {
				base_blocks = List.of(Blocks.STONE, Blocks.DEEPSLATE, Blocks.COBBLED_DEEPSLATE, Blocks.GRANITE);
			}
